...
当前项目处于 V2.5 左右：MVP 闭环、YOLO ONNX 推理框架、目标跟踪、IBVS/PBVS、控制分配、仿真和平台抽象基本具备；双相机融合、相机标定实测、真实硬件协议、MPC/QP、完整运镜 Action 和上层交互还需要继续补齐。
...
C 组当前负责把 Sony RGB 图像变成稳定的 2D 目标轨迹。深度数据和双相机
融合是下一阶段，当前主要对应：
...
本阶段核心输出是 `/perception/tracks`。在完成 Sony/奥比中光标定与融合前，
生产链不得发布 `/perception/targets_3d`，伺服控制默认关闭。
...
src/perception_pkg/config/depth_params.yaml  # 历史仿真兼容，本轮不修改
...
src/perception_pkg/src/yolo_processing.cpp
src/perception_pkg/src/tensorrt_backend.cpp
src/perception_pkg/src/depth_estimator_node.cpp  # 历史仿真兼容
src/sony_camera_pkg/
...
请只阅读 sony_camera_pkg、perception_pkg、Target/TargetArray 接口和 SetTrackingTarget 服务。
请整理：
1. /sony/image_raw 到 /perception/tracks 的完整数据流；
2. detection、tracking 的职责区别；
3. 当前 Sony 驱动、YOLO ONNX/TensorRT 和跟踪分别完成到什么程度；
...
| TrackingNode | 代码收口，待实景验收 | 发布 CONFIRMED/LOST 轨迹、目标选择服务和延迟诊断 |
| MultiObjectTracker | 代码收口 | Kalman + Hungarian 全局 IoU 关联、连续命中确认、类别一致匹配 |
...
目标选择服务验证。默认服务名为 `/tracking_node/set_tracking_target`：

```bash
ros2 service call /tracking_node/set_tracking_target vision_servo_msgs/srv/SetTrackingTarget \
...
| 对接方 | C 组提供 | C 组依赖 |
| --- | --- | --- |
| D 组 | 当前仅提供 `/perception/tracks` 的 bbox、center、tracking_id；不得当作有效 3D 输入 | 融合完成前 D 组保持控制闭环关闭 |
| E 组 | `SetTrackingTarget` 服务，目标 ID/类别选择能力 | E 组需要明确 UI/语音如何选择目标 |
| B 组 | `/sony/image_raw`、`/sony/camera_info` 与 2D 感知状态 | Sony 相机 USB/供电和 Jetson 运行环境 |
...
```text
command_node
  -> 调用 /tracking_node/set_tracking_target
  -> 调用 /servo/set_mode
...
| `/camera/depth/image_raw` | 奥比中光驱动/仿真 | 后续双相机融合节点 |
| `/camera/camera_info` | Sony 驱动/仿真 | C 组、D 组 |
| `/camera/depth/camera_info` | 奥比中光驱动/仿真 | 后续双相机融合节点 |
| `/perception/detections` | C 组 Detection | C 组 Tracking |
| `/perception/tracks` | C 组 Tracking | 调试/可视化、后续双相机融合 |
| `/perception/targets_3d` | mock/历史深度节点/后续融合节点 | D 组 ServoManager |
...
| `/tracking_node/set_tracking_target` | C 组 Tracking | E 组目标选择 |
